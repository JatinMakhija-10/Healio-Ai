...
Usage:
    python scripts/scrape_pa_formulations.py

Output:
    data/ayurveda/processed/pa-formulations.jsonl
"""
...
# ── Config ───────────────────────────────────────────────────────────────────
INDEX_URL     = "https://www.planetayurveda.com/classical-ayurvedic-formulations/"
OUTPUT_DIR    = Path("data/ayurveda/processed")
OUTPUT_FILE   = OUTPUT_DIR / "pa-formulations.jsonl"
PROGRESS_FILE = OUTPUT_DIR / "pa-formulations-progress.json"
...
def make_id(name: str, idx: int) -> str:
    ...
    return f"pa_formula_{slug}_{idx}"
...
def extract_keywords(name: str, text: str) -> list[str]:
    ...
    formula_terms = ["ayurveda", "classical", "formulation", "churna", "vati", "ghrita",
                     "asava", "arishta", "taila", "rasa", "dosha", "vata", "pitta", "kapha"]
...
def detect_section(paragraph: str) -> str:
    ...
    if any(w in lower for w in ["ingredient", "composition", "contain", "formul"]):
        return "Ingredients & Composition"
    if any(w in lower for w in ["benefit", "indication", "used for", "therapeutic"]):
        return "Benefits & Indications"
    if any(w in lower for w in ["dose", "dosage", "direction", "how to use"]):
        return "Dosage & Administration"
    if any(w in lower for w in ["reference", "classical text", "shastra", "samhita"]):
        return "Classical Reference"
    return "Overview"
...
async def get_links(page: Page) -> list[tuple[str, str]]:
    print(f"\n[INFO] Loading formulations index: {INDEX_URL}")
    ...
    for a in soup.find_all('a', href=True):
        ...
        # Formulations index links to /library/ AND /classical-ayurvedic-formulations/ subpages
        is_library = '/library/' in href
        is_formulation = '/classical-ayurvedic-formulations/' in href and href != INDEX_URL
        if (is_library or is_formulation) and href not in seen and len(name) > 2:
            seen.add(href)
            if not href.startswith('http'):
                href = f"https://www.planetayurveda.com{href}"
            links.append((name, href))
    print(f"[OK] Found {len(links)} unique formulation links")
    return links
            ...
            if text_buffer.strip():
                for chunk in chunk_text(text_buffer):
                    chunks_out.append({
                        ...
                        "book": "Classical Ayurvedic Formulations", "category": "classical_formula",
...
async def main():
    ...
    print(f"\n{'='*60}")
    print(f"[DONE] Classical Formulations complete! {total_chunks} chunks -> {OUTPUT_FILE}")
    if failed_urls:
