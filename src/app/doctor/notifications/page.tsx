...
export default function DoctorNotificationsPage() {
    ...
    const handleMarkAllRead = () => {
        if (user?.id) markAllReadMutation.mutate(user.id, { onSuccess: () => refetch() });
    };
        ...
        <div className="space-y-6 max-w-3xl">
            ...
            <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-lg w-fit">
                ...
                >
                    Unread
                    {unreadCount > 0 && (
                        <span className="text-xs px-1.5 py-0.5 rounded-full font-bold bg-red-100 text-red-500">
                            {unreadCount}
                        </span>
                    )}
                </button>
                    ...
                    {isLoading ? (
                        ...
                        <div className="py-16 text-center text-slate-400">
                            ...
                            <p className="text-sm mt-1">
                                {filter === 'unread' ? 'Switch to "All" to see past notifications' : "New bookings and patient updates will appear here"}
                            </p>
                        </div>
                    ) : (
