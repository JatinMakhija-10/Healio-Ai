...
import { getPlatformFeePercentage } from '@/lib/subscription/plans';
...
class InvoiceService {
    ...
    async createInvoice(data: CreateInvoiceData): Promise<Invoice> {
        ...
        const doctorTierFeePercentage = await this.getDoctorPlatformFeePercentage(data.doctor_id);
        const platformFeePercentage =
            doctorTierFeePercentage === 0
                ? 0
                : data.platform_fee_percentage ?? doctorTierFeePercentage;
    ...
    private async getDoctorPlatformFeePercentage(doctorId: string): Promise<number> {
        ...
        if (!doctor?.user_id) {
            return getPlatformFeePercentage('free');
        }
        ...
        return getPlatformFeePercentage(profile?.subscription_plan);
    }
